    ...
    async function Bigquery() {
        ...
        ${query.filter != undefined && query.filterDate != undefined
                ? `where ${Array.isArray(query.filter)
                    ? `${query.filter[0]} ${query.filter[1]} fechaExpedicion ${query.filterDate[0]} and fechaExpedicion ${query.filterDate[1]} order by ${query.order} ${query.typeOrder} limit ${query.limit} offset ${query.page}`
                    : `${query.filter} fechaExpedicion ${query.filterDate[0]} and fechaExpedicion ${query.filterDate[1]}  order by ${query.order} ${query.typeOrder} limit ${query.limit} offset ${query.page}`}`
                : query.filterDate != undefined
                    ? `where fechaExpedicion ${query.filterDate[0]} and fechaExpedicion ${query.filterDate[1]} order by ${query.order} ${query.typeOrder} limit ${query.limit} offset ${query.page}`
